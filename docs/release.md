...
LibraCore does not call a release ready merely because source files exist. A release requires reproducible verification evidence. The current source manifests are prepared for **2.0.12**, and the npm-generated `frontend/package-lock.json` is committed.
...
5. Verify backend:
   ...
   mvn clean verify
   ```
6. Confirm `frontend/package-lock.json` is present, reviewed with any dependency declaration changes, and synchronized with `frontend/package.json`. The 2.0.12 lockfile was generated by the hosted bootstrap workflow after a successful `npm ci` and full frontend quality gate. If a future dependency change makes the lockfile stale, regenerate it with the supported Node/npm toolchain or the **Frontend Lockfile Bootstrap** workflow; never hand-edit or synthesize it.
