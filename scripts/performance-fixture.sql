...
-- Deterministic fictional data for a disposable LibraCore benchmark database.
-- Use scripts/load-performance-fixture.sh; its explicit write/database-name gates
-- are part of the safety contract for this destructive, repeatable fixture.

BEGIN;

-- Remove only deterministic fixture UUID namespaces so reruns have stable shape.
...
DELETE FROM fine_rule WHERE id = 'fcfcfcfc-0000-0000-0000-000000000001';
...
INSERT INTO fine_rule (
    ...
    currency_code, max_renewals, loan_period_days, reservation_hold_days, active
)
VALUES (
    'fcfcfcfc-0000-0000-0000-000000000001',
    'f0f0f0f0-0000-0000-0000-000000000001',
    'Performance fixture circulation policy',
    2.00, 0, 500.00, 'INR', 2, 14, 3, TRUE
);
...
WITH params AS (
    SELECT GREATEST(1, CEIL((:'books')::numeric / 100)::integer) AS count
)
...
FROM params, generate_series(1, count) AS g;

WITH params AS (
    SELECT GREATEST(1, CEIL((:'books')::numeric / 20)::integer) AS count
)
...
WITH params AS (
    SELECT LEAST(50, GREATEST(10, CEIL((:'books')::numeric / 500)::integer)) AS count
)
...
WITH counts AS (
    SELECT GREATEST(1, CEIL((:'books')::numeric / 100)::integer) AS publishers
)
INSERT INTO book (
...
SELECT
    ...
    ('f2f2f2f2-0000-0000-0000-' || lpad((((g - 1) % publishers) + 1)::text, 12, '0'))::uuid
FROM counts, generate_series(1, (:'books')::integer) AS g;

WITH counts AS (
    SELECT GREATEST(1, CEIL((:'books')::numeric / 20)::integer) AS authors
)
INSERT INTO book_author (book_id, author_id, contribution_order, contribution_role)
SELECT
    ('f5f5f5f5-0000-0000-0000-' || lpad(g::text, 12, '0'))::uuid,
    ('f3f3f3f3-0000-0000-0000-' || lpad((((g - 1) % authors) + 1)::text, 12, '0'))::uuid,
    0,
    'AUTHOR'
FROM counts, generate_series(1, (:'books')::integer) AS g;

WITH counts AS (
    SELECT LEAST(50, GREATEST(10, CEIL((:'books')::numeric / 500)::integer)) AS categories
)
INSERT INTO book_category (book_id, category_id)
SELECT
    ('f5f5f5f5-0000-0000-0000-' || lpad(g::text, 12, '0'))::uuid,
    ('f4f4f4f4-0000-0000-0000-' || lpad((((g - 1) % categories) + 1)::text, 12, '0'))::uuid
FROM counts, generate_series(1, (:'books')::integer) AS g;
...
-- Disabled and intentionally non-login-capable; it only satisfies loan audit FKs.
...
INSERT INTO reservation (id, book_id, member_id, pickup_branch_id, status, requested_at)
SELECT
